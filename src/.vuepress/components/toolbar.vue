  ...
  <div class="container">
    ...
    <div class="corner">
      <a href="https://github.com/small-universe/qiqi-tribe" class="github-corner" aria-label="View source on GitHub" target="_blank">
        <svg width="80" height="80" viewBox="0 0 250 250" style="fill:#000504; color:#fff; position: absolute; top: 0; border: 0; right: 0;" aria-hidden="true">
...
export default {
  ...
  mounted() {
    ...
    // 需要在合适的生命周期钩子中动态导入 "../static/vue-layx/layx"
    // 因此需要确保在beforeMount或者mounted访问浏览器/DOM的API。
    import("../static/layx/layx.js").then(module => {
      this.toolbar = module.default
    })
    window.addEventListener("dblclick", this.dbclick);
  },
  methods:{
    onlineBack(){
      layx.iframe('coderunner','在线工具','https://tool.lu/coderunner/',{
        width:800,
        height:450
      });
    },
    rookieBack(){
      layx.iframe('runoob','菜鸟工具','https://c.runoob.com/compile/10',{
        width:600,
        height:450
      });
    },
    ...
    codesandboxWeb(){
      layx.iframe('codesandbox','CodeSandBox','https://codesandbox.io/s/github/small-universe/qiqi-tribe/tree/dev?file=/src/.vuepress/config.js',
          {
            with:1300,
            height:450
          });
    ...
    rookieWeb(){
      layx.iframe('runoob','菜鸟工具','https://c.runoob.com/front-end/61',
      {
        width:600,
        height:500
      });
...
<style scoped>
/** 在style中导入style的固定格式 */
/*@import "../static/new-dream/css/main.css";*/
@import "../static/layx/layx.css";
.container{
  position: fixed;
  right: 0;
  top: 58.5px;
